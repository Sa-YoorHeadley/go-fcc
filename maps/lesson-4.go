...
func getNameCounts(names []string) map[rune]map[string]int {
	...
	for _, name := range names {
		runeValue := rune(name[0])
		_, runeExists := counts[runeValue]
		if(!runeExists){
			counts[runeValue] = map[string]int{}
			counts[runeValue][name] = 1
			continue
		} 
		_, nameExists := counts[runeValue][name]
		if(nameExists){
			counts[runeValue][name] = counts[runeValue][name] + 1
		} else {
			counts[runeValue][name] = 1
		}
